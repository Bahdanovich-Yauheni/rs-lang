...
import { PlayCard, ProgressBar, Statistics } from 'components'
...
const LearnComponent = ({ showModal }) => {
  const {
    learnProgress,
    words,
    cardSettings: { amountOfCards, level, difficultOnly },
  } = useContext(Context)
  ...
  function setNextCard(idx) {
    if (idx >= amountOfCards) {
      return (
        <div className='play-card play-card_statistic'>
          <div className='card-box card-box_statistic'>
            <Statistics initialActive={0} isEnd />
          </div>
        </div>
      )
    ...
    return subWords.map((word, i) => (
      <PlayCard
        key={word.id || word._id}
        word={word.optional ? word.optional : word}
        next={() => {
